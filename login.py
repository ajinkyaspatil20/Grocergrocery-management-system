...
def login_user():
    if user.get() == '' or user.get() == 'Username' or code.get() == '':
        ...
        if row==None:
            ...
            messagebox.showinfo("Welcome","Login Susscessfull")
...
def forgot_password():
    ...
    def change_pass():
        if user.get()=='' or code.get()=='' or conf.get()=='':
            ...
            con=pymysql.connect(host='localhost',user='root',password='Shar@2004',database='userdata')
            mycursor= con.cursor()
    ...
    Button(frame,width=25,pady=7,text='RESET',bg='#006666',activebackground='#006666',activeforeground='white',fg='white',border=0,command=change_pass).place(x=60,y=250)
    fwindow.mainloop()
...
Button(frame,width=39,pady=7,text='Log In',bg='#006666',fg='white',border=0,command=login_user).place(x=35,y=250)


sign_up=Button(frame,width=15,text='Forgot Password?',border=0,bg='white',cursor='hand2',fg='#006666',command=forgot_password)
...
sign_up=Button(frame,width=6,text='Sign Up',border=0,bg='white',cursor='hand2',fg='blue',activeforeground='#8c03fc',activebackground='white',command=sign_up_page)
